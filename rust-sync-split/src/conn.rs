//! Per-connection handling — **"split socket I/O from TLS state" strategy.**
//!
//! This is the experiment described in `../README.md`. The latency in the
//! baseline `rust-lib-sync` crate came from a single per-connection thread that
//! polled because a blocking rustls connection can't be split across two
//! threads. Here we recover the zero-latency two-thread model even for TLS:
//!
//! - The *TCP fd* is genuinely duplex (the kernel allows one thread blocking in
//!   `read()` while another `write()`s), so we `try_clone()` it.
//! - Only the rustls `Connection` is a single shared object, so it lives behind
//!   an `Arc<Mutex>` that is held **only for the microsecond crypto steps**,
//!   never across a blocking socket read.
//!
//! Reader thread: blocks on the cloned fd with no lock, then briefly locks to
//! `read_tls` + `process_new_packets` + drain `reader()`. Writer thread: blocks
//! on the broker's outbound channel (no lock), then briefly locks to
//! `writer().write` + `write_tls`. All `write_tls`/socket writes are serialized
//! by the write-socket mutex so TLS records never interleave. Plain TCP needs
//! none of this — it just uses two `try_clone`d handles directly.
