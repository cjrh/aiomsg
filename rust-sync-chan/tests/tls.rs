...
/// The point of this experiment: the epoll reactor wakes on readiness or on
/// `notify()`, so outbound TLS messages go out immediately — no poll interval.
/// 20 sequential round-trips finish near-instantly.
#[test]
fn tls_round_trips_are_prompt() {
    ...
    assert!(
        ...
        "20 TLS round-trips took {elapsed:?}; expected prompt (event-driven) delivery"
    );
}
