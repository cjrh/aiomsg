...
![Decentralised network](https://upload.wikimedia.org/wikipedia/commons/5/5e/NetworkDecentral.svg)

<sub>Attribution: And1mu [CC BY-SA 4.0 (https://creativecommons.org/licenses/by-sa/4.0)]</sub>
...
[Dynamic Discovery Problem](http://zguide.zeromq.org/page:all#The-Dynamic-Discovery-Problem)).
...
each instance would be placed in a different [availability zones](https://searchaws.techtarget.com/definition/availability-zones).
...
1.  The proxy service is doing very, very little work. Thus, we expect
...
2.  We could compile only the proxy service into faster low-level code using
...
3.  Performance is not the only reason services are dynamically scaled.
    ...
    systems.
4.  A separate proxy service as shown above isolates a really complex
...
5.  We can still run multiple instances of our proxy service using an
...
All you have to do is create an [SSLContext](https://docs.python.org/3/library/ssl.html#ssl.SSLContext)
...
You could also use [Let's Encrypt](https://letsencrypt.org/)
...
The slashed O is used in homage to [ØMQ](http://zeromq.org/), a truly
